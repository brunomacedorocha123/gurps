...
function obterAtributosAtuais() {
    ...
    // Se não tiver funções do sistema principal, tenta obter dos elementos HTML
    const atributos = { ST: 10, DX: 10, IQ: 10, HT: 10, PERC: 10 };
    
    // Procurar elementos de atributos no DOM
    const elementosAtributos = {
        ST: document.getElementById('valor-st') || document.querySelector('[data-atributo="ST"]'),
        DX: document.getElementById('valor-dx') || document.querySelector('[data-atributo="DX"]'),
        IQ: document.getElementById('valor-iq') || document.querySelector('[data-atributo="IQ"]'),
        HT: document.getElementById('valor-ht') || document.querySelector('[data-atributo="HT"]'),
        PERC: document.getElementById('valor-perc') || document.getElementById('valor-percepcao') || document.querySelector('[data-atributo="PERC"]')
    };
    
    for (const [atributo, elemento] of Object.entries(elementosAtributos)) {
        if (elemento) {
            let valorTexto = elemento.textContent || elemento.value || elemento.innerText || '10';
            // Remover símbolos como +, - etc
