// ============================================
// RESUMO-PERICIAS-COMPLETO-ORIGINAL.js
// CÓDIGO COMPLETO ORIGINAL - SÓ SEM LOGS E COM CSS LIMPO
// ============================================
...
function capturarPericiasDireto() {
  try {
    ...
    if (window.estadoPericias && window.estadoPericias.periciasAprendidas) {
      ...
      if (pericias.length > 0) return { pericias, totalPontos };
    }
...
function capturarTecnicasDireto() {
  try {
    ...
    if (window.estadoTecnicas && window.estadoTecnicas.aprendidas) {
      ...
      if (tecnicas.length > 0) return { tecnicas, totalPontos };
    }
...
function atualizarTabelaPericias(pericias) {
  ...
  if (!pericias || pericias.length === 0) {
    tbody.innerHTML = '<tr class="vazio"><td colspan="3">Nenhuma perícia aprendida</td></tr>';
    return;
  }
  ...
  pericias.forEach(pericia => {
    ...
    html += `
      <tr>
        <td class="td-nome" title="${pericia.nome}">${nomeDisplay}</td>
        <td class="td-pontos">${pericia.pontos || 0}</td>
        <td class="td-nh">${pericia.nh || 0}</td>
      </tr>
    `;
  });
...
function atualizarListaTecnicas(tecnicas) {
  ...
  tecnicas.forEach(tecnica => {
    ...
    nomeDisplay = nomeDisplay.replace(/<[^>]*>/g, '');
    if (nomeDisplay.length > 28) nomeDisplay = nomeDisplay.substring(0, 25) + '...';
...
function criarTabelaPericias() {
  ...
  thead.innerHTML = '<tr><th>PERÍCIA</th><th class="th-nivel">PONTOS</th><th class="th-pontos">NH</th></tr>';
...
// CSS LIMPO (SEM AS CORES FEIAS QUE EU METI)
function aplicarEstilosLimpos() {
  const styleId = 'resumo-estilos-limpos';
  ...
  style.textContent = `
    ...
    #tabelaPericiasResumo .td-nome { color: #ddd; font-size: 0.8rem; padding: 6px 8px; max-width: 150px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
    #tabelaPericiasResumo .td-pontos { color: #ffd700; font-weight: 700; font-size: 0.85rem; text-align: center; padding: 6px 4px; background: rgba(255, 215, 0, 0.15); border-radius: 4px; min-width: 45px; }
    #tabelaPericiasResumo .td-nh { color: #2ecc71; font-weight: 800; font-size: 0.9rem; text-align: center; padding: 6px 4px; background: rgba(46, 204, 113, 0.15); border-radius: 4px; min-width: 45px; }
    .tecnica-resumo-item { display: flex; justify-content: space-between; align-items: center; padding: 8px 10px; margin-bottom: 6px; background: rgba(155, 89, 182, 0.1); border-radius: 6px; border-left: 3px solid #9b59b6; }
    .tecnica-resumo-item:hover { background: rgba(155, 89, 182, 0.2); }
    .tecnica-resumo-item .tecnica-nome { flex: 1; color: #eee; font-size: 0.85rem; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; padding-right: 10px; }
    .tecnica-resumo-item .tecnica-pontos { color: #ffd700; font-weight: 700; font-size: 0.85rem; padding: 4px 8px; background: rgba(255, 215, 0, 0.15); border-radius: 12px; min-width: 40px; text-align: center; margin-right: 8px; }
    .tecnica-resumo-item .tecnica-nh { color: #2ecc71; font-weight: 800; font-size: 0.9rem; padding: 4px 10px; background: rgba(46, 204, 113, 0.15); border-radius: 12px; min-width: 45px; text-align: center; }
  `;
